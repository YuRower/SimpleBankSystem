...
public class ATM extends javax.swing.JFrame implements KeyListener {
	...
	private void initComponents() {
		...
		statusField.setText(
				"Добро пожаловать в мой банк! Вставте карту , введите идентификатор клиента и нажмите клавишу ENTER...");
	...
	private void enterButtonActionPerformed(java.awt.event.ActionEvent evt) {
		...
		try {
			...
			historyArea.append("Клиент с ID = " + (customerID + 1) + " - " + currentCustomer.getLastName() + ", "
	...
	private void balanceButtonActionPerformed(java.awt.event.ActionEvent evt) {
		...
		try {
			historyArea
					.append("Баланс  " + currentCustomer.getFirstName() + " на счету $" + currentAccount.getBalance());
		} catch (Exception e) {
			statusField.setText("Нажмите Enter");
		}
		if (currentAccount instanceof CheckingAccount) {
			historyArea.append(". Эта учетная запись с защитой от овердрафта $"
					+ ((CheckingAccount) currentAccount).getOverdraftAmount() + "\n");
		} else {
			historyArea.append(". Это сберегательный счет с процентной ставкой "
